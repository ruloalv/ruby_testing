...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    let(:random_number) {double('random_number', value: 3)}
    subject(:game_over) {described_class.new(0, 9, random_number, 3)}
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_over) {described_class.new(0, 9, random_number, 4)}
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      subject(:edge_case) { described_class.new(5, 8, random_number, 7)}
      it 'updates min to the same value as max' do
        expect {edge_case.update_range}.to change {edge_case.min}.to(8)
      end

      it 'does not update max' do
        expect {edge_case.update_range}.not_to change {edge_case.max}
      end
    end
  end
end
